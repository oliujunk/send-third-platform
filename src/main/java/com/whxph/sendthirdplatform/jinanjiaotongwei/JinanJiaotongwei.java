...
public class JinanJiaotongwei {
    ...
    private static final Map<Integer, Channel> deviceMap = new ConcurrentHashMap<>();
    ...
    private final String username = "jiaotongwei";

    private Set<JSONObject> deviceSet = new HashSet<>();
    ...
    public void updateDeviceList() throws InterruptedException {
        if (start) {
            ...
            deviceSet = OldXphapi.updateDevice1(restTemplate, username, token);
            updateChannel();
        }
    }
    ...
    private void login(Integer deviceId) {
        ...
        String deviceName = channelData.getDeviceName();
        ...
        dataList.add((byte) 0);
        // 终端ID
        dataList.add(HexBinDecOct.hex2bcd(Byte.parseByte(deviceName.substring(0, 2))));
        dataList.add(HexBinDecOct.hex2bcd(Byte.parseByte(deviceName.substring(2, 4))));
        dataList.add(HexBinDecOct.hex2bcd(Byte.parseByte(deviceName.substring(4, 6))));
        dataList.add(HexBinDecOct.hex2bcd(Byte.parseByte(deviceName.substring(6, 8))));
        dataList.add(HexBinDecOct.hex2bcd(Byte.parseByte(deviceName.substring(8, 10))));
        dataList.add(HexBinDecOct.hex2bcd(Byte.parseByte(deviceName.substring(10, 12))));
        // 消息流水号
        ...
        LOGGER.info("登录: {}", ByteBufUtil.hexDump(sendData).toUpperCase());
    ...
    private void heartBeat(Integer deviceId) {
        ...
        sendData[dataListSize + 1] = (byte) (checkSum >> 8);
        sendData[dataListSize + 2] = (byte) 0x16;
        LOGGER.info("心跳: {}", ByteBufUtil.hexDump(sendData).toUpperCase());
    ...
    private void monitorData(Integer deviceId) {
        ...
        sendData[dataListSize + 1] = (byte) (checkSum >> 8);
        sendData[dataListSize + 2] = (byte) 0x16;
        LOGGER.info("数据上报： {}", ByteBufUtil.hexDump(sendData).toUpperCase());
        NettyUtils.sendDataToChannel(sendData, deviceMap.get(deviceId));
    }
    ...
    private void updateChannel() throws InterruptedException {
        ...
        deviceMap.clear();
        onlineMap.clear();
        for (JSONObject device : deviceSet) {
            ChannelFuture channelFuture = client.connect("221.214.51.19", 39105).sync();
            Channel channel = channelFuture.channel();
            int deviceId = device.getInteger("facId");
            String deviceName = device.getString("facName");
            deviceMap.put(deviceId, channel);
            ChannelData channelData = new ChannelData(deviceId, channel, (short) 0, false, deviceName);
            onlineMap.put(channel, channelData);
            login(deviceId);
            Thread.sleep(1000);
        }
